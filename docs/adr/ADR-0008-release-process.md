...
## Amendment A1 — 2026-09-19: publishing waits for every check

The release workflow no longer runs its own check step: it calls the reusable check workflow on
the tagged commit and publishes only after every job in it passes, and it refuses a tag that is
