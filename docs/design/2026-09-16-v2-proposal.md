---
title: The v2 proposal (2026-09-16)
doc_type: design
status: approved
updated: '2026-09-16'
---

> **Historical record.** This is the assessment of fence.js 1.0.1 and the modernization plan the
> owner approved on 2026-09-16, kept as written (with its implementation notes appended). The
> decisions it led to are the authoritative records: [ADR-0001](../adr/ADR-0001-immutable-registry-builder-with-inferred-types.md)
> through [ADR-0008](../adr/ADR-0008-release-process.md). Where this document and an ADR disagree,
> the ADR wins. The 2.0 release also removed the compatibility aliases this plan proposed keeping
> ([ADR-0005](../adr/ADR-0005-no-compatibility-surface-in-2-0.md)).
...
- **Private fields (`#`)** replace `_underscore` conventions. Tests that poked `_invokables`/`_name` will be rewritten against public behavior.
...
| **1. Toolchain swap**   | TypeScript, Vitest, ESLint 9, tsdown, `exports`, Actions matrix, hooks. Port existing JS to `.ts` with `any` where needed, no behavior change. | `2.0.0-alpha.1` (types shipped, behavior identical)           | 1 day     |
...
Behavioral defects C1–C9 were confirmed with a throwaway Jest probe (removed afterwards); each row in §2.2 quotes the observed value.
