...
Ten offline checks over the governed files, implemented in
...
| `changelog`    | `CHANGELOG.md`, the hand-written record through 2.0.2, keeps Keep a Changelog's shape: `## [x.y.z] - YYYY-MM-DD` headings, versions descending, only the six categories, and no `[Unreleased]` section, because release notes are generated from commits since (ADR-0013).                   |
| `ignore-files` | The root `.gitignore` is the only ignore list (plus the root `.prettierignore`, for tracked files Prettier must not rewrite): any other `.gitignore`, `.eslintignore`, `.prettierignore` or `.npmignore` is refused, because ESLint, Prettier and the gate walker would read it differently. |
...
Tracked files plus untracked files the root `.gitignore` does not ignore
(`git ls-files --cached --others --exclude-from=.gitignore`), with tracked files deleted from the
working tree dropped. Only the root `.gitignore` applies — not `.git/info/exclude`, a global
excludes file or nested `.gitignore` files — because it is the one list ESLint and Prettier read
too, and a local build, an agent worktree or a scratch file can
