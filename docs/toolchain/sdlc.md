...
| → `review`          | Every criterion `satisfied: true` with an `evidence` link that resolves; `npm run check` green (the pre-push hook runs it on a clean export of the commit; CI runs it on both Node lines).                                                                      |
