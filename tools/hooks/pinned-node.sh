...
pinned_node_missing_message() {
  if command -v mise >/dev/null 2>&1; then
    ...
    echo "Node $PINNED_NODE (mise.toml) is not installed, the project's mise.toml is not trusted, or mise is older than its min_version, so $1 did not run. In the repository run: mise trust && mise install (upgrade mise first if it says so)${mise_said:+ (mise said: $mise_said)}"
  else
